...
const Calculator = () => {
    ...
    const btnClick = (item) => {
        ...
        if (item.action === BTN_ACTIONS.DELETE1) {
            // Xóa ký tự cuối cùng của expression
            const updatedExpression = expression.slice(0, -1);
            setExpression(updatedExpression);

            // Xóa toàn bộ nội dung hiện tại của expDiv
            expDiv.innerHTML = '';

            // Thêm lại từng ký tự của chuỗi expression sau khi xóa ký tự cuối
            updatedExpression.split('').forEach(char => {
                addAnimSpan(char);
            });
        }
